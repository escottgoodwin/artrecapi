...
def recent_arts_mongo(trans_lang,days):
    colnm = trans_lang + '_vec'
    uri = 'mongodb://root:g6rbir74JYNs@35.230.48.17:27017'
    gclient = pymongo.MongoClient(uri)
    gdb = gclient['langavecs']
    gcol = gdb[colnm]
    ago = datetime.now() - timedelta(days=days)
    interval_rslts = gcol.find({"date": {"$gt": ago}},projection={'_id':False})
    interval_rslts1 =[[x['art_id'],pickle.loads(x['vec'])] for x in interval_rslts]
    return interval_rslts1
...
def friend_list(user_id_friend,status):
    conn = psycopg2.connect("dbname='langalearn' user='ymroddi' host='lango84.cukbl7fyxfht.us-west-1.rds.amazonaws.com' password='royallord8413'")
    cur = conn.cursor()
    sql="SELECT userid1,userid2 FROM relationships WHERE (userid1 = "+str(user_id_friend)+" OR userid2 = "+str(user_id_friend)+") AND status = " +str(status)
    cur.execute(sql)
    ...
    for x in friend_rslt:
        if x[0] != user_id_friend:
            friends_list.append(x[0])
        if x[1] != user_id_friend:
            friends_list.append(x[1])
...
def load_models_s3(lang):
    ...
    KEY4 = lang + 'model3.model.docvecs.vectors_docs.npy'
    ...
    if not os.path.exists(KEY1):
        ...
        try:
            ...
            print(KEY3)
            s3.Bucket(bucket_name).download_file(KEY4, KEY4)
            print(KEY4)
...
enlang_model = load_models_s3('en')
print('en models loaded')
frlang_model = load_models_s3('fr')
print('fr models loaded')
eslang_model = load_models_s3('es')
print('es models loaded')
delang_model = load_models_s3('de')
print('de models loaded')
...
@application.route("/apis/link_search_mg", methods=['POST'])
def link_search_mg():
    ...
    #load to time matrix
    vec_range = recent_arts_mongo(trans_lang,int(daterange))
...
def link_search():
    ...
    conn = psycopg2.connect(dbconnect)
    cur = conn.cursor()
    col_nm = trans_lang + '_arts'
    sql="SELECT link,title,art_id FROM " + col_nm + " WHERE art_id IN %s"
    cur.execute(sql,(sims2,))
    recs = cur.fetchall()
    dictrecs = [{'link':x[0],'title':x[1],'art_id':x[2]} for x in recs]
    conn.close()
...
def friends_search():
    ...
    if len(age_src)>0:
        ...
        age_qry = ''
    sql="SELECT name,native_lang,sex,residence,age,(now() - last_login) FROM user_ids WHERE sex = '" + gender + "' AND native_lang = '" + srch_native_lang + "'" + age_qry
    cur.execute(sql)
    friend_rslt = cur.fetchall()
    conn.close()
    friends = [{'name':item[0],'native_lang':item[1],'gender':item[2],'residnce':item[3],'age':str(item[4]),"last_login_time":str(item[5].days) + ' days'} for item in friend_rslt]
...
def friends_relationship():
    ...
    if len(user_friends) > 0:
        payload = fetch_recs_id(user_friends)
        resp=json.dumps(payload)
    else:
        payload = []
        resp=json.dumps(payload)
...
def friend_request():
    ...
    if req_type == 'friend_request':
        ...
        status = str(1)
        pendingq = "('"+ user_id_friend + "','"+ requested_id + "','"+ status + "','"+ request_date + "')"
        sql='INSERT INTO relationships (userid1,userid2,status,request_date) VALUES' + pendingq
        message = "Request Made " + str(request_date)
    if req_type == 'friend_acceptance':
        status = str(2)
        accept_date  = datetime.now()
        acceptq = "('"+ status + "','"+ accept_date + "') WHERE userid2 = "+ user_id_friend +" AND userid1 = "+ requested_id + + ")"
        sql='UPDATE relationships (status,accept_date) VALUES' + acceptq
        message = "Request Accepted "+ str(request_date)
    cur.execute(sql)
    conn.commit()
    conn.close()
